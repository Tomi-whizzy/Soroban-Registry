...
fn get_binary_path() -> PathBuf {
    ...
    let p = PathBuf::from(&manifest).join("target").join("debug").join(&exe);
    if p.exists() { return p; }
    PathBuf::from(&manifest).parent()
...
fn test_notification_subscribe_list_unsubscribe() {
    ...
    let sub = Command::new(get_binary_path())
        .args(["contract", "notification", "subscribe", addr,
               "--alerts", "updates,security", "--channels", "cli", "--frequency", "instant"])
        .output().unwrap();
    assert!(sub.status.success(), "subscribe failed: {}", String::from_utf8_lossy(&sub.stderr));
    ...
    let unsub = Command::new(get_binary_path())
        ...
        .output().unwrap();
    assert!(unsub.status.success(), "unsubscribe failed: {}", String::from_utf8_lossy(&unsub.stderr));
...
fn test_notification_invalid_alert_type() {
    let out = Command::new(get_binary_path())
        .args(["contract", "notification", "subscribe", "CSOME_ADDR",
               "--alerts", "invalid_type"])
        .output().unwrap();
    assert!(!out.status.success());
    let stderr = String::from_utf8_lossy(&out.stderr);
